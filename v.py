# ==============================================================================
# 🤟 BIM "V" Gesture Description:
# - Hand Orientation: Right hand, palm facing the camera directly.
# - Index & Middle Fingers: Fully straight and spread apart to form a V shape; restrict the distance between index landmark 8 and middle landmark 12 to keep them separated.
...
class GestureChecker(BaseGesture):
    def __init__(self):
        ...
        # 🎯 Geometric thresholds calibrated for the outward flaring "V" profile
        self.THRESHOLDS = {
            ...
            # 👐 Separation Lock: Minimum normalized distance required between Index Tip (8) and Middle Tip (12)
            # This forces the fingers apart to explicitly distinguish "V" from "U"
            "index_to_middle_tip_min": 0.35,
    ...
    def check_gesture(self, hands_list: list, current_step: int):
        ...
        # 4. Separation Check: Index Tip (8) and Middle Tip (12) must flare away from each other
        tip_proximity = self._dist(lm[8], lm[12]) / hand_size
        tips_separated = tip_proximity >= self.THRESHOLDS["index_to_middle_tip_min"]
        ...
        all_ok = palm_valid and angles_ok and lengths_ok and tips_separated and other_fingers_folded and thumb_anchored
        ...
        if self.enable_debug_log:
            print("\n============================================================")
            print(f"📊 [V Gesture Profile] Real-Time Runtime Metrics")
            ...
            print(f"  [4. V-Shape Separation (8 to 12)]: Dist={tip_proximity:.3f} (Req: > {self.THRESHOLDS['index_to_middle_tip_min']:.3f}) -> Status: {tips_separated}")
